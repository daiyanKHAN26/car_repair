...
if(count($errors)==0){
    ...
    $mechID = $_REQUEST['mechID'];
	$sql = "INSERT INTO  bookapp (AppoID, Date, userID,mechID) VALUES ('$AppoID','$Date','$userprofile','$mechID') ";
...
if(count($errors)==0){
	...
	$sqladd = "INSERT INTO  mechanic (mechanicID, mechanicname, email, Address, ContactNumber, password,category) VALUES ('$addID','$addname','$addEmail','$addAddress','$addContactNumber','$addPassword','$addcategory') ";
...
if (isset($_POST['Delete'])) {
 ...
 if (count($errors)==0) {
	...
	$querydelete="DELETE FROM mechanic WHERE mechanicID='$deleteID' ";
	if ($mysqli -> query($querydelete)) {

		if ($mysqli->affected_rows==0) {
			 array_push($errors,"Wrong mechanic ID");
...
<!-- $query1="SELECT category FROM mechanic";
